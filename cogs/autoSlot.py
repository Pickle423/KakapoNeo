...
class autoSlot(commands.Cog):
    ...
    async def addSlots(self, ctx, operation_id, *, slots):
        ...
        # Additionally, create reminders and events for the operation - THIS FEATURE HAS BEEN SUSPENDED
        '''
    ...
    async def iaslot(self, ctx, slot_id, target=None):
        ...
        if self.database['operations'][operation_id]['assignments'].get(slot_id):
            return await botCommandsChannel.send(f"{ctx.user.mention} cannot claim slot {slot_id} because it is already taken.")
