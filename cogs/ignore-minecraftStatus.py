...
class server():
    async def pingServer():
        async with aiohttp.ClientSession() as session:
                async with session.get("https://api.mcsrvstat.us/2/minecraft.pickle423.com") as request :
...
class minecraftStatus(commands.Cog):
    ...
    async def change_status(self):
        ...
        await self.client.change_presence(activity=nextcord.Game(f"Modded MC: {serverc}/{serverm}"))

    @commands.command(aliases=['status','serverstatus', 'players', 'online'])
    async def server(self, ctx):
        serverc, serverm, serverl = await server.pingServer()
        statusEmbed = nextcord.Embed(title="Server Status", description=f"Pickloninacraft's Population is currently {serverc}/{serverm}", color=0x00ff00)
        statusEmbed.add_field(name="Players:", value=serverl, inline=False)
        statusEmbed.set_footer(text="Contact pickle423 for issues regarding the server.")
